module LightningApi
  class Resolver
    ...
    def resolve(params:)
      _dataset = if dataset && dataset.respond_to?(inferred_dataset_method)
    ...
    private
      ...
      def raise_inferred_class_error
        raise InferredClassError, "Please define a 'resolve' method, because we cannot infer a dataset class"
      end
  end
end